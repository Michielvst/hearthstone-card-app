...
class App extends React.Component<{}> {
  ...
  loadCards = async (e: any) => {
    ...
    this.setState({
      cards: selectedCards.filter((card: any) => {
        return card["img"];
      }),
    });
    this.setTypes();
  ...
  loadSets = async (data: any) => {
    ...
    const sets = info["sets"].filter(
      (el: any) => el !== "Promo" && el !== "System" && el !== "Debug"
    );
    console.log(sets);
    this.setState({
      sets: sets,
  ...
  renderCards = () => {
    ...
    return Array.from(cards).map((el: any) => (
    ...
    return (
      <React.Fragment>
        <div className="appHeader">
          <img
            src="https://s3-storage.textopus.nl/wp-content/uploads/2015/09/18031825/Hearthstone-iPhone-iPad.jpg"
            className="backimg"
            alt="logo"
          />
          <ExpansionPicker
            ...
            setTypesFilter={this.setTypesFilter}
          />
        </div>
